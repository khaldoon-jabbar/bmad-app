...
import type { ProjectState, Phase, Epic, RecentAction } from '../../shared/types.js';
...
function inferPhase(state: ProjectState): Phase | null {
  if (state.sprint) return 'implementation';
  if (state.epics.length > 0) return 'solutioning';
  if (state.documents.prd) return 'planning';
  if (state.documents.projectContext) return 'analysis';
  return null;
}

export async function getDashboardState(projectPath: string): Promise<ProjectState> {
...
async function deriveRecentActions(bmadOutput: string, state: ProjectState): Promise<RecentAction[]> {
  ...
  for (const epic of state.epics) {
    for (const story of epic.stories) {
      if (story.status !== 'draft') {
        ...
        try { const stat = await fs.stat(storyPath); ts = stat.mtime.toISOString(); } catch {}
