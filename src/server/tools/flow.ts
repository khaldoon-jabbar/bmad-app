import type { FlowGraph, FlowNode, FlowEdge, Track, FlowNodeStatus, ProjectState } from '../../shared/types.js';
import { getDashboardState } from './dashboard.js';

function getBmadFlowNodes(): FlowNode[] {
  return [
...
function getBmadFlowEdges(): FlowEdge[] {
  return [
...
function getQuickFlowNodes(): FlowNode[] {
  return [
...
function getQuickFlowEdges(): FlowEdge[] {
  return [{ id: 'e-i-qd', source: 'intent', target: 'quick-dev' }];
}

const PHASES_ORDER = ['analysis', 'planning', 'solutioning', 'implementation'] as const;

function applyLiveStatus(nodes: FlowNode[], state: ProjectState): FlowNode[] {
  ...
  return nodes.map((node): FlowNode => {
    ...
    if (node.type === 'phase') {
      ...
      if (nodeIdx < currentIdx) status = 'done';
      else if (nodeIdx === currentIdx) status = 'active';
    } else {
      switch (node.triggerCode) {
        case 'AB': status = state.documents.projectContext ? 'done' : 'pending'; break;
        case 'CP': status = state.documents.prd ? 'done' : 'pending'; break;
        case 'CU': status = state.documents.uxSpec ? 'done' : 'pending'; break;
        case 'CA': status = state.documents.architecture ? 'done' : 'pending'; break;
        case 'CE': status = state.epics.length > 0 ? 'done' : 'pending'; break;
        case 'SP': status = state.sprint ? 'done' : 'pending'; break;
        case 'DS': status = state.sprint ? 'in-progress' : 'pending'; break;
        default: break;
      }
    }
...
export async function handleFlow(track: Track, projectPath: string): Promise<FlowGraph> {
