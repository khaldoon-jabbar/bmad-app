...
const VALID_STATUSES = ['draft', 'in-progress', 'review', 'done', 'blocked'];
function parseAcceptanceCriteria(body) {
    ...
    for (const line of lines) {
        ...
        if (inACSection && /^#{1,3}\s/.test(line))
            break;
        if (inACSection) {
...
export function parseStoryFile(filename, content) {
    ...
    const status = VALID_STATUSES.includes(frontmatter['status'])
        ? frontmatter['status']
        : 'draft';
